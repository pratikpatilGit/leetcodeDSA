     ...
     public ArrayList<Integer> diagonal(Node root)
      {
        ...
        while (!queue.isEmpty()) {
            ...
            while (temp != null) {
                result.add(temp.data); // Add the data to the result list

                // Enqueue the left child if it exists (for the next diagonal)
                if (temp.left != null) {
                    queue.offer(temp.left);
                }

                // Move to the right child to continue on the same diagonal
                temp = temp.right;
            }
        }
        return result;
